function _pnpm_install --on-event pnpm_install
  ...
  read --command="$global_dir" --prompt="set_color green; echo -n PNPM global-dir; set_color normal; echo -n ': ';" global_dir
  command npm set global-dir "$global_dir"
  read --command="$store_dir" --prompt="set_color green; echo -n PNPM store-dir; set_color normal; echo -n ': ';" store_dir
  command npm set store-dir "$store_dir"

  # prepend global-dir to PATH to expose global binaries
  contains $global_dir/bin $fish_user_paths || set --prepend fish_user_paths $global_dir/bin
...
function _pnpm_uninstall --on-event pnpm_uninstall
  ...
  test -d "$global_dir" && rm -rf "$global_dir"
  test -d "$store_dir" && rm -rf "$store_dir"
